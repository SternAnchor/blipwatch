...
describe("HTTP API", () => {
  ...
  it("serves health, latest image, latest metadata, and replay endpoints", async () => {
    ...
    await sendUdpPacket(radarPort, Buffer.from("42575331010104d207d00004000f80ff", "hex"));
    await waitFor(async () => {
    ...
    const replayFrame = await fetch(`${baseUrl}/radar/replay/frame?at=${encodeURIComponent(framesBody.frames[0]?.capturedAt ?? "")}`);
